...
const toolDetailFile = join(rootDir, 'tool-detail.html')
...
function createAssetVersion(...sources) {
  return createHash('sha256').update(sources.join('\n')).digest('hex').slice(0, 12)
}

function replaceScriptVersions(source, version) {
  return source
    .replace(/src="generated-content\.js(?:\?v=[^"]*)?"/g, `src="generated-content.js?v=${version}"`)
    .replace(/src="toolbox\.js(?:\?v=[^"]*)?"/g, `src="toolbox.js?v=${version}"`)
}
...
async function main() {
  ...
  const generated = `// This file is generated by scripts/generate-content.js. Edit content/tools/*.md instead.\nwindow.toolboxContent = {\n  toolsData: ${JSON.stringify(tools, null, 2)},\n  toolDetails: ${JSON.stringify(details, null, 2)},\n  categories: ${JSON.stringify(categories, null, 2)}\n}\n`
  const toolboxSource = await readFile(rootToolboxFile, 'utf8')
  const assetVersion = createAssetVersion(generated, toolboxSource)
  ...
  const nextIndex = replaceScriptVersions(
  ...
  const currentToolDetail = await readFile(toolDetailFile, 'utf8')
  await writeFile(toolDetailFile, replaceScriptVersions(currentToolDetail, assetVersion), 'utf8')
